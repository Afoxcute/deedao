...
export const getPoolKey = async () => {
  const [poolKey] = await asyncGetPda([Buffer.from(RS_PREFIX)], PROGRAM_ID);
  return poolKey;
};
...
export const getStakedNFTKey = async (nftMintPk) => {
  const [stakedNftKey] = await asyncGetPda(
    [Buffer.from(RS_STAKE_SEED), nftMintPk.toBuffer()],
    PROGRAM_ID
  );
  return stakedNftKey;
};

export const getStakeInfoKey = async (nftMintPk) => {
  const [stakedNftKey] = await asyncGetPda(
    [Buffer.from(RS_STAKEINFO_SEED), nftMintPk.toBuffer()],
    PROGRAM_ID
  );
  return stakedNftKey;
};
