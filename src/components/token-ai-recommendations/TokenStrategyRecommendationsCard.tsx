...
interface TokenStrategyRecommendation {
...
interface TokenStrategyRecommendationsCardProps {
  recommendations: TokenStrategyRecommendation[];
  confidence: number;
}

const getStrategyIcon = (type: string) => {
  switch (type) {
    case 'TOKEN_STAKING': return '🔒';
    case 'TOKEN_VESTING': return '⏰';
...
const getStrategyColor = (type: string) => {
  switch (type) {
    case 'TOKEN_STAKING': return '#4caf50';
    case 'TOKEN_VESTING': return '#66BB6A';
    case 'POOL_CREATION': return '#2196f3';
    case 'LIQUIDITY_PROVISION': return '#9c27b0';
    default: return '#757575';
  }
};

export const TokenStrategyRecommendationsCard: React.FC<TokenStrategyRecommendationsCardProps> = ({ 
  recommendations, 
  confidence 
}) => {
          ...
          <Accordion key={index} sx={{ border: `1px solid ${theme.palette.divider}` }}>
            ...
            <AccordionDetails>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                ...
                {/* Token Allocation */}
                {recommendation.tokenAllocation && recommendation.tokenAllocation.length > 0 && (
                  <Box>
                    <Typography variant="h6" sx={{ marginBottom: '12px' }}>
                      Recommended Token Allocation
                    </Typography>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                      {recommendation.tokenAllocation.map((allocation, allocIndex) => (
                        <Box
                          key={allocIndex}
                          sx={{
                            display: 'flex',
