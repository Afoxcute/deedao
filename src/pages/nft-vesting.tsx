...
import { NFTVestingMarketList } from '../components/nft-vesting/NFTVestingMarketList';
import { useSettings } from '../contexts';
import { useWallet } from '../contexts/wallet';
import { usePool, usePoolMeta, usePoolOracle } from '../hooks/api';
import theme from '../theme';

export default function NFTVestingPage() {
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: '1200px', margin: '0 auto', padding: '24px' }}>
      <Row sx={{ justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <Typography variant="h1">NFT Vesting</Typography>
      </Row>
      <Divider />
      ...
      {!walletId ? (
        <Box sx={{ 
          ...
          <Typography variant="h2">Connect your wallet to view NFT vesting details</Typography>
        </Box>
      ) : (
        <>
          {hasData && (
            <Section width={SectionSize.FULL} sx={{ padding: '6px', display: 'flex', flexDirection: 'column' }}>
              <NFTVestingMarketList poolId={poolMeta?.id ?? ''} />
            </Section>
          )}
        </>
      )}
