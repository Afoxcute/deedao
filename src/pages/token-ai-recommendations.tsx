...
import { TokenAIRecommendationPanel } from '../components/token-ai-recommendations/TokenAIRecommendationPanel';
...
function TabPanel(props: TabPanelProps) {
  ...
  return (
    <div
      role="tabpanel"
      hidden={value !== index}
      id={`token-ai-tabpanel-${index}`}
      aria-labelledby={`token-ai-tab-${index}`}
      {...other}
    >
...
export default function TokenAIRecommendationsPage() {
  ...
  const getRequestType = (tabIndex: number) => {
    switch (tabIndex) {
      case 0: return 'TOKEN_STAKING_ANALYSIS';
      case 1: return 'TOKEN_VESTING_ANALYSIS';
      case 2: return 'POOL_CREATION';
      case 3: return 'PORTFOLIO_OPTIMIZATION';
      default: return 'TOKEN_STAKING_ANALYSIS';
    }
  };
    ...
    return (
      <Box sx={{ 
        ...
        <Typography variant="body1" color="text.secondary" sx={{ textAlign: 'center', marginBottom: '24px' }}>
          Please connect your wallet to access token AI investment recommendations.
        </Typography>
      ...
      <Box sx={{ maxWidth: '1400px', margin: '0 auto' }}>
        ...
        <Paper sx={{ marginBottom: '24px' }}>
          <Tabs
            ...
            aria-label="Token AI recommendation tabs"
            sx={{
              '& .MuiTab-root': {
                fontSize: '16px',
                fontWeight: 500,
        ...
        <TabPanel value={tabValue} index={0}>
          <TokenAIRecommendationPanel
            currentStakingPositions={[]}
            currentVestingSchedules={[]}
            availablePools={[]}
            requestType="TOKEN_STAKING_ANALYSIS"
          />
        </TabPanel>
        ...
        <TabPanel value={tabValue} index={1}>
          <TokenAIRecommendationPanel
            ...
            currentVestingSchedules={[]}
            availablePools={[]}
            requestType="TOKEN_VESTING_ANALYSIS"
          />
        </TabPanel>
        ...
        <Box sx={{ marginTop: '32px', padding: '24px', backgroundColor: theme.palette.background.paper, borderRadius: '12px' }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 2 }}>
            <Box>
              <Typography variant="h6" color="primary.main">Impermanent Loss Analysis</Typography>
              <Typography variant="body2" color="text.secondary">
                Advanced analysis of impermanent loss risks for liquidity provision strategies
              </Typography>
            </Box>
            <Box>
              <Typography variant="h6" color="primary.main">Token Volatility Assessment</Typography>
              <Typography variant="body2" color="text.secondary">
                Token-specific risk analysis considering price volatility and market conditions
              </Typography>
            </Box>
            <Box>
              <Typography variant="h6" color="primary.main">Yield Optimization</Typography>
              <Typography variant="body2" color="text.secondary">
                Strategies to maximize yield while managing risks across different tokens
              </Typography>
            </Box>
            <Box>
              <Typography variant="h6" color="primary.main">Liquidity Management</Typography>
              <Typography variant="body2" color="text.secondary">
                Recommendations for optimal liquidity provision and pool selection
              </Typography>
            </Box>
          </Box>
        </Box>
